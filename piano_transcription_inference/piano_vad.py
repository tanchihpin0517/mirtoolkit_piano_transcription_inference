...
def note_detection_with_onset_offset_regress_stream(
    frame_output,
    onset_output,
    onset_shift_output,
    offset_output,
    ...
    bgn = prev_state.get('bgn', None)
    frame_disappear = prev_state.get('frame_disappear', None)
    offset_occur = prev_state.get('offset_occur', None)
    prev_end = prev_state.get('prev_end', 0)

    for i in range(prev_end, prev_end+onset_output.shape[0]):
        if onset_output[i-prev_end] == 1:
            """Onset detected"""
            if bgn:
                """Consecutive onsets. E.g., pedal is not released, but two
                consecutive notes being played."""
                fin = max(i - 1, prev_end)

                output_tuples.append([
                    bgn,
                    fin,
                    prev_state['onset_shift'],
                    0,
                    prev_state['velocity']
                ])
                frame_disappear, offset_occur = None, None
            ...
            bgn = i
            prev_state['onset_shift'] = onset_shift_output[bgn-prev_end]
            prev_state['velocity'] = velocity_output[bgn-prev_end]

        if bgn and i > bgn:
            """If onset found, then search offset"""
            if frame_output[i-prev_end] <= frame_threshold and not frame_disappear:
                """Frame disappear detected"""
                frame_disappear = i
                prev_state['offset_shift_dis'] = offset_shift_output[i-prev_end]

            if offset_output[i-prev_end] == 1 and not offset_occur:
                """Offset detected"""
                offset_occur = i
                prev_state['offset_shift_occ'] = offset_shift_output[i-prev_end]

            if frame_disappear:
                if offset_occur and offset_occur - bgn > frame_disappear - offset_occur:
                    """bgn --------- offset_occur --- frame_disappear"""
                    fin = offset_occur
                    offset_shift = prev_state['offset_shift_occ']
                else:
                    """bgn --- offset_occur --------- frame_disappear"""
                    fin = frame_disappear
                    offset_shift = prev_state['offset_shift_dis']
                ...
                bgn, frame_disappear, offset_occur = None, None, None
                prev_state['incomplete'] = None

            if bgn and i-prev_end == onset_output.shape[0] - 1:
                fin = i
                prev_state['incomplete'] = [
                    bgn,
                    fin,
                    prev_state['onset_shift'],
                    offset_shift_output[fin-prev_end],
                    prev_state['velocity'],
                ]

            if bgn and (i - bgn >= 600):
                """Offset not detected"""
                fin = i
                output_tuples.append([
                    bgn,
        ...
        prev_state['prev_end'] = prev_state.get('prev_end', 0) + 1

    prev_state['bgn'] = bgn
    prev_state['frame_disappear'] = frame_disappear
    prev_state['offset_occur'] = offset_occur

    return output_tuples


def pedal_detection_with_onset_offset_regress_stream(
    frame_output,
    offset_output,
    offset_shift_output,
    frame_threshold,
    ...
    frame_disappear = prev_state.get('frame_disappear', None)
    offset_occur = prev_state.get('offset_occur', None)
    prev_end = prev_state.get('prev_end', 1)

    # for i in range(1, frame_output.shape[0]):
    for i in range(prev_end, prev_end+frame_output.shape[0]):
        if frame_output[i-prev_end] >= frame_threshold and \
                frame_output[i-prev_end] > frame_output[i - prev_end - 1]:
            """Pedal onset detected"""
            if bgn:
                pass
            else:
        ...
        if bgn and i > bgn:
            ...
            if offset_occur:
                fin = offset_occur
                offset_shift = prev_state['offset_shift_occ']
                output_tuples.append([bgn, fin, 0., offset_shift])
                bgn, frame_disappear, offset_occur = None, None, None
        ...
        prev_state['prev_end'] = prev_state.get('prev_end', 1) + 1
